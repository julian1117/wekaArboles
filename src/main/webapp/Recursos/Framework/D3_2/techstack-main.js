...
d3.json("../Resources/data/AbonadosTelefoniaMovil.json", function (info) {
    processData(info);
});
...
//    alert(temp);
    if (info.data.length > 0) {
        for (var x = 0; x < info.data.length; x++) {
            ...
            data.push({
                cat: 'Total abonados', name: info.data[x][8], value: parseInt(info.data[x][11]),
                icon: info.data[x][24],
                desc: 'Total abonados: ' + info.data[x][11]
            });
        }

        for (var x = 0; x < info.data.length; x++) {
            data.push({
                cat: 'Total prepago', name: info.data[x][8], value: parseInt(info.data[x][14]),
                icon: info.data[x][24],
                desc: 'Total abonados: ' + info.data[x][14]
            });
        }

        for (var x = 0; x < info.data.length; x++) {
            data.push({
                cat: 'Total pospago', name: info.data[x][8], value: parseInt(info.data[x][17]),
                icon: info.data[x][24],
                desc: 'Total pospago: ' + info.data[x][17]
            });
        }

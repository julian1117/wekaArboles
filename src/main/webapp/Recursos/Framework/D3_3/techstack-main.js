...
d3.json("../Resources/data/portuario.json", function (info) {
    objectMobile = info;
    processData(info);
});
...
//    var temp = info.data[0][0];
//    alert(temp);
    if (info.data.length > 0) {
        for (var x = 0; x < info.data.length - 1; x++) {

            data.push({
                cat: 'Importaciones', name: info.data[x][8], value: parseInt(info.data[x][9]),
                icon: info.data[x][8],
                desc: 'Total importacion: ' + parseFloat(info.data[x][9])
            });

            data.push({
                cat: 'Exportaciones', name: info.data[x][8], value: parseInt(info.data[x][10]),
                icon: info.data[x][8],
                desc: 'Total exportacion: ' + parseFloat(info.data[x][10])
            });
...
// legend 1
        ...
        let sizeScale = d3.scaleOrdinal()
                .domain(['Menos importaciones - Exportaciones', 'Mas importaciones - Exportaciones'])
                .range([5, 10]);
